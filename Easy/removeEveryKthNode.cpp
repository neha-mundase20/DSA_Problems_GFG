...
Easy Accuracy: 29.88% Submissions: 84K+ Points: 2
...
class Solution {
    ...
    Node* deleteK(Node *head,int K){
      if (head==nullptr || K==1) {
            // If K is 1, all nodes need to be deleted, hence return nullptr.
            return nullptr;
        }

        Node*current=head;
        Node*previous=nullptr;
        int index=1;  // 1-based index for easier understanding
        ...
        while (current!=nullptr) {
            if (index % K==0) {
                ...
                if (previous!=nullptr) {
                    ...
                    delete current;  // Freeing memory if necessary in C++
                    current=previous->next;  // Move to next node after deletion
                }
            } else {
                previous=current;
